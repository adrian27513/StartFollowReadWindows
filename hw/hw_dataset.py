...
def collate(batch):
    ...
    final = []
    for npl in all_labels:
        label = np.array(np.pad(npl, (0, max_len - len(npl)), 'constant', constant_values=0))
        final.append(label)
    final = np.vstack(tuple(final))
    ...
    labels = torch.from_numpy(final.astype(np.int32))
