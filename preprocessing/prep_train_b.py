...
def handle_single_image(xml_path, img_path, output_directory, config={}):
    ...
    with open(xml_path, encoding='utf-8') as f:
        lines = f.readlines()
        num_lines = sum(1 for line in lines if len(line.strip())>0)
...
def process_dir(xml_directory, img_directory, output_directory):
    ...
    for i, filename in enumerate(list(to_process)):
        ...
        all_ground_truth.append([json_path, img_path[3:]])

    return all_ground_truth

if __name__ == "__main__":
    xml_directory = '../data/Train-B'
    img_directory = '../data/Train-B'

    output_directory = 'data/train_b'
    training_output_json = 'data/train_b_training_set.json'
    validation_output_json = 'data/train_b_validation_set.json'
